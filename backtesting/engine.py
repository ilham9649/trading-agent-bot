...
def save_backtest_result(result: BacktestResult, output_dir: str = "./backtesting/results") -> str:
    """Save backtest result to timestamped folder.
    ...
    folder_name = (
        f"{timestamp}_{result.config.symbol}_"
        f"{result.config.start_date}_to_{result.config.end_date}"
    )
    ...
    if result.daily_portfolio_values:
        portfolio_file = folder_path / "portfolio_value.csv"
        with open(portfolio_file, 'w') as f:
            f.write("Date,Portfolio Value\n")
            for date, value in result.daily_portfolio_values:
                f.write(f"{date},{value}\n")
    
    # Update index file
    update_backtest_index(output_dir, result, folder_name, timestamp)
    
    logger.info(f"Backtest results saved to {folder_path}")
    return str(folder_path)


def update_backtest_index(base_dir: str, result: BacktestResult, folder_name: str, timestamp: str):
    """Update the backtest index file.
    
    Args:
        base_dir: Base results directory
        result: BacktestResult
        folder_name: Name of result folder
        timestamp: Timestamp of backtest
    """
    index_file = Path(base_dir) / "BACKTEST_INDEX.md"
    
    # Create entry
    entry = f"""
### {timestamp.replace('_', ' at ')} - {result.config.symbol}
...
"""
    ...
    # Append to index (or create if doesn't exist)
    if index_file.exists():
        content = index_file.read_text()
        # Insert after header
        if "# Backtest Index" in content:
            parts = content.split("---\n", 1)
            content = parts[0] + "---\n" + entry + (parts[1] if len(parts) > 1 else "")
        else:
            content = "# Backtest Index\n\n" + entry + content
    else:
        content = "# Backtest Index\n\nAll backtests are listed below in reverse chronological order.\n\n---\n" + entry
